...
class Profile(models.Model):
    ...
    icon_file = models.ImageField(
        "アイコン(ファイル)", upload_to="profile_icons/", blank=True, null=True
    )
    ...
    current_avatar = models.CharField(
        "現在のアバター", max_length=100, default="初期アバター", blank=True
    )
...
class Post(models.Model):
    ...
    image_file = models.ImageField(
        "画像(ファイル)", upload_to="post_images/", blank=True, null=True
    )
    ...
    liked_by = models.ManyToManyField(
        User, related_name="liked_posts", blank=True, verbose_name="いいねしたユーザー"
    )
...
class Comment(models.Model):
    post = models.ForeignKey(
        Post, on_delete=models.CASCADE, related_name="comments", verbose_name="投稿"
    )
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="ユーザー")
...
class Notification(models.Model):
    ...
    post = models.ForeignKey(
        Post, on_delete=models.CASCADE, null=True, blank=True, verbose_name="関連投稿"
    )
...
class UserLoginSession(models.Model):
    ...
    last_seen_at = models.DateTimeField(
        "最終アクセス日時", auto_now=True, db_index=True
    )
