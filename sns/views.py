...
NAMES_LIST = ["ゆいちゃん", "たいき", "りゅう", "みな", "そら", "はる", "れん", "あお", "なぎ"]
WORDS_LIST = [
    "闇落ちした", "限界突破の", "集中の", "知識を喰らう", "夜明けの", "継続する",
    "努力する", "計算爆走", "眠気討伐", "図書館の", "覚醒した", "静かなる",
]
...
def edit_profile(request):
    ...
    if request.method == "POST":
        ...
        if "create_custom_title" in request.POST:
            ...
            if word and name:
                full_title = f"{name}{word}" if order == "name_first" else f"{word}{name}"
                new_item, created_item = GachaItem.objects.get_or_create(
