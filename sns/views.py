...
def is_site_admin(user):
    return user.is_authenticated and (
        user.is_superuser or user.username == settings.SITE_ADMIN_USERNAME
    )
...
def set_post_date(post, study_date):
    ...
    if study_date == today:
        ...
        dt = timezone.make_aware(
            datetime.combine(study_date, time(hour=12)), timezone.get_current_timezone()
        )
...
def is_icon_item(item):
    return (
        item.name.startswith("【アイコン】")
        or "アイコン" in item.name
        or "繧｢繧､繧ｳ繝ｳ" in item.name
    )
...
def owned_title_parts(items):
    ...
    return {
        "words": [
            word for word in WORDS_LIST if any(word in name for name in display_names)
        ],
        "nouns": [
            noun for noun in NOUNS_LIST if any(noun in name for name in display_names)
        ],
        "names": [
            name for name in NAMES_LIST if any(name in title for title in display_names)
        ],
    }
...
def build_stacked_week_chart(user):
    ...
    raw = (
        Post.objects.filter(
            user=user, created_at__date__gte=start, subject__in=subject_names
        )
        ...
        datasets.append(
            {
                "label": subject,
                "data": [
                    totals.get((start + timedelta(days=i), subject), 0)
                    for i in range(7)
                ],
...
def index(request):
    ...
    if request.method == "POST":
        ...
        if "comment_text" in request.POST:
            ...
            Comment.objects.create(
                post=post, user=request.user, text=request.POST.get("comment_text")
            )
        ...
        if "delete_comment_id" in request.POST:
            comment = get_object_or_404(
                Comment, id=request.POST.get("delete_comment_id"), user=request.user
            )
            comment.delete()
            return redirect("index")
        ...
        if "delete_post_id" in request.POST:
            post = get_object_or_404(
                Post, id=request.POST.get("delete_post_id"), user=request.user
            )
            post.delete()
            return redirect("index")
    ...
    all_posts = (
        base_query.filter(content__icontains=search_query)
        if search_query
        else base_query.all()
    )
    ...
    for post in posts.object_list:
        if hasattr(post.user, "profile"):
            post.current_rarity = rarity_by_name.get(
                post.user.profile.current_title, "N"
            )
            post.avatar_rarity = rarity_by_name.get(
                post.user.profile.current_avatar, "N"
            )
            post.current_title_display = item_display_name(
                post.user.profile.current_title
            )
        else:
    ...
    if profile.target_date and profile.target_minutes > 0:
        has_target = True
        total_study = (
            Post.objects.filter(user=request.user).aggregate(Sum("study_minutes"))[
                "study_minutes__sum"
            ]
            or 0
        )
        remaining_display = format_study_time(
            max(profile.target_minutes - total_study, 0)
        )
...
def edit_profile(request):
    ...
    icon_query = (
        Q(name__contains="【アイコン】")
        | Q(name__contains="アイコン")
        | Q(name__contains="繧｢繧､繧ｳ繝ｳ")
    )
        ...
        title_rows = [
            {
                ...
                "can_sell": item.name != profile.current_title
                and not is_refined_item(item),
        ...
        avatar_rows = [
            {
                ...
                "display_name": item_display_name(item),
                "sell_value": SELL_VALUES.get(item.rarity, 0),
                "can_sell": item.name != profile.current_avatar
                and not is_refined_item(item),
                "is_refined": is_refined_item(item),
            }
        ...
        return render(
            ...
            "sns/edit_profile.html",
            {
                ...
                "buy_rows": [
                    {
                        "rarity": rarity,
                        "cost": BUY_COSTS[rarity],
                        "sell": SELL_VALUES[rarity],
                    }
    ...
    if request.method == "POST":
        if "update_profile" in request.POST:
            ...
            if requested_username and requested_username != request.user.username:
                if (
                    User.objects.filter(username=requested_username)
                    .exclude(id=request.user.id)
                    .exists()
                ):
                    username_error = (
                        "このIDはすでに使われています。別のIDにしてください。"
                    )
                else:
        ...
        if "combine_titles" in request.POST:
            ...
            if item_a and item_b and item_a.id != item_b.id:
                left, right = (
                    (item_display_name(item_b), item_display_name(item_a))
                    if order == "reverse"
                    else (item_display_name(item_a), item_display_name(item_b))
                )
                base_name = f"{left}{right}"
                full_title = f"精錬:{base_name[:95]}"
                max_rarity_val = max(
                    RARITY_ORDER.get(item_a.rarity, 1),
                    RARITY_ORDER.get(item_b.rarity, 1),
                )
                new_item, created_item = GachaItem.objects.get_or_create(
                    name=full_title,
                    defaults={"rarity": RARITY_BY_VALUE[max_rarity_val]},
                )
        ...
        if "refine_parts" in request.POST:
            ...
            if (word or noun or name) and valid_word and valid_noun and valid_name:
                ...
                new_item, created_item = GachaItem.objects.get_or_create(
                    name=f"精錬:{full_title[:47]}",
                    defaults={"rarity": RARITY_BY_VALUE[rarity_value]},
                )
                profile.items.add(new_item)
        ...
        if "sell_selected" in request.POST:
            sell_items(
                profile, profile.items.filter(id__in=request.POST.getlist("sell_items"))
            )
            return redirect("edit_profile")
        ...
        if "buy_rarity" in request.POST:
            ...
            if cost and profile.exchange_points >= cost:
                ...
                item_name = (
                    ...
                    else (allowed_names[0] if allowed_names else shop_item_name(rarity))
                )
                item, created_item = GachaItem.objects.get_or_create(
                    name=item_name, defaults={"rarity": rarity}
                )
...
def like_post_ajax(request, post_id):
    ...
    return JsonResponse({"liked": not liked, "count": post.liked_by.count()})
...
def user_profile(request, username):
    ...
    total_minutes = (
        target_posts.aggregate(Sum("study_minutes"))["study_minutes__sum"] or 0
    )
    month_start = timezone.localdate().replace(day=1)
    month_minutes = (
        target_posts.filter(created_at__date__gte=month_start).aggregate(
            Sum("study_minutes")
        )["study_minutes__sum"]
        or 0
    )
...
def notifications_view(request):
    ...
    return render(
        request, "sns/notifications.html", {"notifications": notifs, "unread_count": 0}
    )
...
def analytics_view(request):
    ...
    user_posts = Post.objects.filter(
        user=request.user, study_minutes__gt=0, created_at__date__range=(start, end)
    )
    subject_rows = build_subject_rows(user_posts)
    total_minutes = (
        user_posts.aggregate(Sum("study_minutes"))["study_minutes__sum"] or 0
    )

    day_rows = (
        user_posts.annotate(day=TruncDate("created_at"))
        .values("day")
        .annotate(total=Sum("study_minutes"))
    )
...
def build_ranking(start, end, limit=30):
    ...
    users = {
        ...
        for user in User.objects.select_related("profile").filter(
            id__in=[row["user"] for row in rows]
        )
    }
    ranking = []
...
def admin_login_activity(request):
    ...
    sessions = UserLoginSession.objects.select_related("user").order_by("-login_at")[
        :100
    ]
    active_count = 0
    for session in sessions:
        session.is_active_now = (
            session.logout_at is None and session.last_seen_at >= active_cutoff
        )
        if session.is_active_now:
            active_count += 1
    return render(
        request,
        "sns/admin_login_activity.html",
        {
            "sessions": sessions,
